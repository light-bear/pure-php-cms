...
final class Decoder
{
    ...
    private function read($data, &$offset, $depth)
    {
        ...
        $size = strlen($data);
        $start = $offset;
        if ($offset >= $size) throw new CmsException('Truncated ASN.1 tag');
        ...
        if ($tag === 0x1f) {
            $tag = 0;
            do {
                if ($offset >= $size) throw new CmsException('Truncated ASN.1 high tag');
                $byte = ord($data[$offset++]);
                if ($tag > 0x1ffffff) throw new CmsException('ASN.1 tag is too large');
                $tag = ($tag << 7) | ($byte & 0x7f);
            } while ($byte & 0x80);
        }
        if ($offset >= $size) throw new CmsException('Truncated ASN.1 length');
        ...
        if (!$indefinite) {
            if (($lengthByte & 0x80) === 0) {
                ...
                if ($count === 0 || $count > 4 || $offset + $count > $size) throw new CmsException('Invalid ASN.1 length');
                $length = 0;
                for ($i = 0; $i < $count; $i++) $length = ($length << 8) | ord($data[$offset++]);
            }
            $contentEnd = $offset + $length;
            if ($contentEnd > $size) throw new CmsException('ASN.1 value exceeds input');
        } else {
            $contentEnd = null;
        }
        $contentStart = $offset;
        ...
        if ($constructed) {
            ...
            while ($indefinite ? true : $offset < $contentEnd) {
                ...
                if ($indefinite && $offset + 2 <= $size && $data[$offset] === "\0" && $data[$offset + 1] === "\0") {
                    $contentEnd = $offset;
                    $offset += 2;
                    break;
                }
                $children[] = $this->read($data, $offset, $depth + 1);
            }
